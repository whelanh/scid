...
static Game *scratchGame = nullptr;       // "scratch" game for searches, etc.
...
static OpTable *reports[2] = {nullptr, nullptr};

static void checkSpellChkReady(bool block = false);

const int MAX_EPD = 4;                  // Max simultaneously open EPD windows.
static EpdBook *epdBooks[MAX_EPD] = {nullptr, nullptr, nullptr, nullptr};
...
//    constant string.
inline int setResult(Tcl_Interp *ti, const char *str) {
...
inline int setIntResult(Tcl_Interp *ti, int i) {
  char temp[20];
  sprintf(temp, "%d", i);
...
inline int setUintResult(Tcl_Interp *ti, uint i) {
  char temp[20];
  sprintf(temp, "%u", i);
  Tcl_SetResult(ti, temp, TCL_VOLATILE);
  return TCL_OK;
...
inline int appendUintResult(Tcl_Interp *ti, uint i) {
  char temp[20];
  sprintf(temp, "%u", i);
...
inline uint appendUintElement(Tcl_Interp *ti, uint i) {
  char temp[20];
  sprintf(temp, "%u", i);
...
inline int setUintWidthResult(Tcl_Interp *ti, uint i, uint width) {
  char temp[20];
...
//    Tcl interpreter result.
inline int appendCharResult(Tcl_Interp *ti, char ch) {
  char tempStr[4];
  tempStr[0] = ch;
  tempStr[1] = 0;
  Tcl_AppendResult(ti, tempStr, nullptr);
  return TCL_OK;
}
...
//
inline const char *translate(Tcl_Interp *ti, const char *name,
...
inline int errorResult(Tcl_Interp *ti, errorT err, const char *errorMsg = 0) {
  if (errorMsg != 0)
    Tcl_SetResult(ti, (char *)errorMsg, TCL_STATIC);
  ASSERT(err != OK);
...
int sc_base_export(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  const char *options[] = {"-append",   "-starttext",      "-endtext",
                           "-comments", "-variations",     "-spaces",
                           "-symbols",  "-indentComments", "-indentVariations",
    ...
    for (gamenumT i = 0, n = db->numGames(); i < n; i++) {
      if (db->dbFilter->Get(i)) {    // Export this game:
        if (++numSeen % 1024 == 0) { // Update the percentage done bar:
          if (!progress.report(numSeen, numToExport))
            break;
        }
...
int sc_base_piecetrack(ClientData, Tcl_Interp *ti, int argc,
                       const char **argv) {
  const char *usage = "Usage: sc_base piecetrack [-g|-t] <minMoves> <maxMoves> "
                      "<startSquare ...>";
...
bool checkDuplicate(scidBaseT *base, const IndexEntry *ie1,
                    const IndexEntry *ie2, dupCriteriaT *cr) {
  ...
  if (ie1->GetNumHalfMoves() <= 2 || ie2->GetNumHalfMoves() <= 2) {
    if (ie1->GetYear() == ie2->GetYear() && ie1->GetSite() == ie2->GetSite() &&
...
UI_res_t sc_base_duplicates(scidBaseT *dbase, UI_handle_t ti, int argc,
  ...
  const char *options[] = {
      "-players",   "-colors",    "-event",    "-site",       "-round",
      "-year",      "-month",     "-day",      "-result",     "-eco",
      "-moves",     "-skipshort", "-comments", "-variations", "-setfilter",
      "-usefilter", "-delete",    nullptr};
  enum {
    OPT_PLAYERS,
    OPT_COLORS,
    OPT_EVENT,
  ...
  HFilter filter =
      setFilterToDups ? dbase->getFilter("dbfilter") : HFilter(&tmp_filter);
  filter.clear();
  Progress progress = UI_CreateProgress(ti);
  // Now check same-hash games for duplicates:
  for (size_t i = 0; i < n_hash; i++) {
    if ((i % 1024) == 0) {
      if (!progress.report(i, numGames))
        break;
    }
...
int sc_clipbase(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  static const char *options[] = {"clear", "paste", nullptr};
  enum { CLIP_CLEAR, CLIP_PASTE };
  int index = -1;
...
static ecoTranslateT *ecoTranslations = nullptr;
...
int sc_eco(ClientData cd, Tcl_Interp *ti, int argc, const char **argv) {
  int index = -1;
  static const char *options[] = {"base",    "game",      "read", "reset",
                                  "summary", "translate", nullptr};
  enum { ECO_BASE, ECO_GAME, ECO_READ, ECO_RESET, ECO_SUMMARY, ECO_TRANSLATE };
...
int sc_eco_base(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  if (!db->inUse)
    return errorResult(ti, ERROR_FileNotOpen);
...
int sc_eco_summary(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  Tcl_AppendResult(ti, dstr->Data(), nullptr);
  delete temp;
  delete dstr;
  return TCL_OK;
}
...
void translateECO(Tcl_Interp *ti, const char *strFrom, DString *dstrTo) {
  ...
  while (trans != nullptr) {
    if (trans->language == lang && strContains(dstrTo->Data(), trans->from)) {
...
// Klimmek: or sc_filter freq baseId filterName moves <lowerhalfMove>
// <higherhalfMove>
...
int sc_filter_freq(scidBaseT *dbase, const HFilter &filter, Tcl_Interp *ti,
  ...
  const char *usage = "Usage: sc_filter freq baseId filterName date|elo|move "
                      "<startDate|minElo|lowerhalfMove> "
                      "[<endDate|maxElo|higherhalfMove>] [GuessElo]";

  bool eloMode = false;
  bool moveMode = false;
  bool guessElo = true;
  const char *options[] = {"date", "elo", "move", nullptr};
  ...
  if (eloMode) {
    ...
    // Klimmek: count games with x Moves minMove=NumberHalfmove and maxMove
    // Numberhalfmove+1
    for (uint gnum = 0, n = dbase->numGames(); gnum < n; gnum++) {
      const IndexEntry *ie = dbase->getIndexEntry(gnum);
...
// TODO:
// This functions do not works because they do not specify the base, the filter
// and the sort criteria So for the moment we assume base=db, filter=dbFilter
// and sort=N+
...
int sc_filter_stats(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  for (uint i = 0, n = db->numGames(); i < n; i++) {
    ...
    if (filter.get(i)) {
      if (max == 0) { // Old Statistic :
        ...
        // count all games where player with highest Elo is in the specific
        // range
        if (statType == STATS_ELO) {
          if (inv_max) {
            ...
            // count all games where player with lowest Elo is in the specific
            // range
            uint mini = ie->GetWhiteElo();
            if (mini > ie->GetBlackElo())
              mini = ie->GetBlackElo();
  ...
  char temp[80];
  uint percentScore =
      results[RESULT_White] * 2 + results[RESULT_Draw] + results[RESULT_None];
  percentScore = total ? percentScore * 500 / total : 0;
  sprintf(temp, "%7u %7u %7u %7u   %3u%c%u%%", total, results[RESULT_White],
          results[RESULT_Draw], results[RESULT_Black], percentScore / 10,
          decimalPointChar, percentScore % 10);
  Tcl_AppendResult(ti, temp, nullptr);
  return TCL_OK;
}
...
int sc_filter_old(ClientData cd, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  static const char *options[] = {
      "count",    "first", "frequency", "last",    "negate",    "next",
      "previous", "stats", "search",    "release", "treestats", "export",
      "copy",     "and",   "or",        "new",     nullptr};
  enum {
    FILTER_COUNT,
    FILTER_FIRST,
    FILTER_FREQ,
  ...
  if (argc < 4)
    return errorResult(ti, "Usage: sc_filter <cmd> baseId filterName");
  auto dbase = DBasePool::getBase(strGetUnsigned(argv[2]));
  if (dbase == nullptr)
    return errorResult(ti, "sc_filter: invalid baseId");
  HFilter filter = dbase->getFilter(argv[3]);
  if (filter == 0)
    return errorResult(ti, "sc_filter: invalid filterName");
  switch (index) {
  case FILTER_AND:
    if (argc == 5) {
  ...
  case FILTER_SEARCH:
    if (argc >= 5) {
      ...
      if (subcmd == "board") {
        if (argc == 5 || argc == 6) {
          ...
          if (!SearchPos(pos).setFilter(*dbase, filter, UI_CreateProgress(ti)))
            return UI_Result(ti, ERROR_UserCancel);
    ...
    return errorResult(
        ti,
        "Usage: sc_filter search baseId filterName <header|board|tags> [args]");
  ...
  case FILTER_EXPORT:
    ...
    return errorResult(ti, "Usage: sc_filter export baseId filterName sortCrit "
                           "filename <PGN|LaTeX> [header] [footer]");
  }
...
int sc_game(ClientData cd, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  case GAME_VARIANT:
    return UI_Result(
        ti, OK, db->game->currentPos()->isChess960() ? "chess960" : "standard");
...
static inline bool isCrosstableGame(const IndexEntry *ie, idNumberT siteID,
                                    idNumberT eventID, dateT eventDate) {
...
int sc_game_crosstable(ClientData, Tcl_Interp *ti, int argc,
  ...
  static const char *options[] = {"plain",  "html",  "hypertext",
                                  "filter", "count", nullptr};
  enum { OPT_PLAIN, OPT_HTML, OPT_HYPERTEXT, OPT_FILTER, OPT_COUNT };
  int option = -1;

  const char *usageMsg =
      "Usage: sc_game crosstable plain|html|hypertext|filter|count "
      "[name|rating|score|country] [allplay|swiss] "
      "[(+|-)(colors|countries|tallies|ratings|titles|groups|breaks|numcolumns)"
      "]";
  ...
  for (int arg = 3; arg < argc; arg++) {
    ...
    switch (extraOption) {
    case EOPT_ALLPLAY:
      mode = CROSSTABLE_AllPlayAll;
      break;
    case EOPT_KNOCKOUT:
      mode = CROSSTABLE_Knockout;
      break;
    case EOPT_SWISS:
      mode = CROSSTABLE_Swiss;
      break;
    case EOPT_AUTO:
      mode = CROSSTABLE_Auto;
      break;
    case EOPT_SORT_NAME:
      sort = EOPT_SORT_NAME;
      break;
    case EOPT_SORT_RATING:
      sort = EOPT_SORT_RATING;
      break;
    case EOPT_SORT_SCORE:
      sort = EOPT_SORT_SCORE;
      break;
    case EOPT_SORT_COUNTRY:
      sort = EOPT_SORT_COUNTRY;
      break;
    case EOPT_AGES_OFF:
      showAges = false;
      break;
    case EOPT_AGES_ON:
      showAges = true;
      break;
    case EOPT_BREAKS_OFF:
      showBreaks = false;
      break;
    case EOPT_BREAKS_ON:
      showBreaks = true;
      break;
    case EOPT_COLORS_OFF:
      showColors = false;
      break;
    case EOPT_COLORS_ON:
      showColors = true;
      break;
    case EOPT_COUNTRIES_OFF:
      showCountries = false;
      break;
    case EOPT_COUNTRIES_ON:
      showCountries = true;
      break;
    case EOPT_FLAGS_OFF:
      showFlags = false;
    ...
    case EOPT_TALLIES_OFF:
      showTallies = false;
      break;
    case EOPT_TALLIES_ON:
      showTallies = true;
      break;
    case EOPT_RATINGS_OFF:
      showRatings = false;
      break;
    case EOPT_RATINGS_ON:
      showRatings = true;
      break;
    case EOPT_TITLES_OFF:
      showTitles = false;
      break;
    case EOPT_TITLES_ON:
      showTitles = true;
      break;
    case EOPT_GROUPS_OFF:
      scoreGroups = false;
      break;
    case EOPT_GROUPS_ON:
      scoreGroups = true;
      break;
    case EOPT_DELETED_OFF:
      useDeletedGames = false;
      break;
    case EOPT_DELETED_ON:
      useDeletedGames = true;
      break;
    case EOPT_NUMCOLUMNS_OFF:
      numColumns = false;
      break;
    case EOPT_NUMCOLUMNS_ON:
      numColumns = true;
      break;
    case EOPT_GNUMBER:
      ...
      arg++;
      break;
    case EOPT_THREEWIN_OFF:
      threewin = false;
      break;
    case EOPT_THREEWIN_ON:
      threewin = true;
      break;
    default:
      return errorResult(ti, usageMsg);
  ...
  if (db->getNameBase()->FindExactName(NAME_EVENT, g->GetEventStr(),
                                       &eventId) != OK) {
    return TCL_OK;
  }
  if (db->getNameBase()->FindExactName(NAME_SITE, g->GetSiteStr(), &siteId) !=
      OK) {
    return TCL_OK;
  }
  ...
  if (sort == EOPT_SORT_NAME) {
    ctable->SortByName();
  }
  if (sort == EOPT_SORT_RATING) {
    ctable->SortByElo();
  }
  if (sort == EOPT_SORT_COUNTRY) {
    ctable->SortByCountry();
  }
  ...
  switch (option) {
  case OPT_PLAIN:
    ctable->SetPlainOutput();
    break;
  case OPT_HTML:
    ctable->SetHtmlOutput();
    break;
  case OPT_HYPERTEXT:
  ...
  for (uint i = 0, n = db->numGames(); i < n; i++) {
    const IndexEntry *ie = db->getIndexEntry(i);
    if (ie->GetDeleteFlag() && !useDeletedGames) {
      continue;
    }
    ...
    if (ctable->AddPlayer(whiteId, whiteName, ie->GetWhiteElo(), spell) != OK ||
      ...
      if (!tableFullMessage) {
        ...
        Tcl_AppendResult(
            ti, "Warning: Player limit reached; table is incomplete\n\n", nullptr);
      }
      continue;
    }

    uint round =
        strGetUnsigned(db->getNameBase()->GetName(NAME_ROUND, ie->GetRound()));
    dateT date = ie->GetDate();
    resultT result = ie->GetResult();
    ctable->AddResult(i + 1, whiteId, blackId, result, round, date);
    if (date < firstSeenDate) {
      firstSeenDate = date;
    }
  ...
  uint apaLimit = 300;
  if (mode == CROSSTABLE_AllPlayAll && ctable->NumPlayers() > apaLimit &&
      !tableFullMessage) {
    Tcl_AppendResult(ti,
                     "Warning: Too many players for all-play-all; try "
                     "displaying as a swiss tournament.\n\n",
                     nullptr);
  }
  ...
  strTrimDate(stemp);
  Tcl_AppendResult(ti, stemp, nullptr);
  if (lastSeenDate != firstSeenDate) {
    date_DecodeToString(lastSeenDate, stemp);
    strTrimDate(stemp);
    Tcl_AppendResult(ti, " - ", stemp, nullptr);
  }
  Tcl_AppendResult(ti, newlineStr, nullptr);
  ...
  Tcl_AppendResult(ti, dstr.c_str(), nullptr);
  delete ctable;
  return TCL_OK;
}
...
int sc_game_find(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  if (db->numGames() > gnum) {
    uint score = 0;
    const IndexEntry *ie = db->getIndexEntry(gnum);
    if (ie->GetWhite() == white) {
      score++;
    }
    if (ie->GetBlack() == black) {
      score++;
    }
    if (ie->GetSite() == site) {
      score++;
    }
    if (ie->GetRound() == round) {
      score++;
    }
    if (ie->GetYear() == year) {
      score++;
    }
    if (ie->GetResult() == result) {
      score++;
    }
    if (score == 6) {
      return setUintResult(ti, gnum + 1);
...
int sc_game_firstMoves(ClientData, Tcl_Interp *ti, int argc,
                       const char **argv) {
  if (argc != 3) {
  ...
  if (plyCount < 0)
    plyCount = db->game->GetCurrentPly();
  if (plyCount == 0)
    plyCount = 1;
...
int sc_game_import(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  if (pgn.log.empty())
    return UI_Result(ti, OK, "PGN text imported with no errors or warnings.");

  return UI_Result(ti, OK, "Errors/warnings importing PGN text:\n\n" + pgn.log);
}
...
int sc_game_info(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  while (arg < argc) {
    if (strIsPrefix(argv[arg], "-hideNextMove")) {
      ...
      Tcl_AppendResult(ti, temp, nullptr);
      return TCL_OK;
      // nextMoveNT is the same as nextMove, except that the move is not
      // translated
      ...
      Tcl_AppendResult(ti, temp, nullptr);
      return TCL_OK;
      // previousMoveNT is the same as previousMove, except that the move is not
      // translated
  ...
  std::snprintf(temp, sizeof(temp), "%c%s %u:  <pi %s>%s</pi>",
                toupper(gameStr[0]), gameStr + 1, db->gameNumber + 1,
  ...
  if (db->game->GetEco() != 0) {
    ...
    Tcl_AppendResult(ti, "   <blue><run ::windows::eco::Refresh ", basicEcoStr,
                     ">", fullEcoStr, "</run></blue>", nullptr);
  }
  char dateStr[20];
  date_DecodeToString(db->game->GetDate(), dateStr);
  strTrimDate(dateStr);
  Tcl_AppendResult(ti, "   <red>", dateStr, "</red>", nullptr);

  if (db->gameNumber >= 0) {
    // Check if this game is deleted or has other user-settable flags:
    const IndexEntry *ie = db->getIndexEntry(db->gameNumber);
    if (ie->GetDeleteFlag()) {
      Tcl_AppendResult(ti, "   <gray>(", translate(ti, "deleted"), ")</gray>",
                       nullptr);
    }
    char userFlags[16];
    if (ie->GetFlagStr(userFlags, nullptr) != 0) {
      ...
      if (*flagStr != 0) {
        Tcl_AppendResult(ti, "   <gray>(", translate(ti, "flags", "flags"),
                         ": ", flagStr, nullptr);
        int flagCount = 0;
        while (*flagStr != 0) {
          ...
          case 'W':
            ...
            break;
          case 'B':
            flagName = "BlackOpFlag";
            break;
          case 'M':
            flagName = "MiddlegameFlag";
            break;
          case 'E':
            flagName = "EndgameFlag";
            break;
          case 'N':
            flagName = "NoveltyFlag";
            break;
          case 'P':
            flagName = "PawnFlag";
            break;
          case 'T':
            flagName = "TacticsFlag";
            break;
          case 'Q':
            flagName = "QsideFlag";
            break;
          case 'K':
            ...
            break;
          case '!':
            flagName = "BrilliancyFlag";
            break;
          case '?':
            flagName = "BlunderFlag";
            break;
          case 'U':
            flagName = "UserFlag";
            break;
          ...
          if (flagName != nullptr) {
            Tcl_AppendResult(ti, (flagCount > 0 ? ", " : " - "),
                             translate(ti, flagName), nullptr);
          }
          flagCount++;
          flagStr++;
        }
        Tcl_AppendResult(ti, ")</gray>", nullptr);
      }
    }

    if (db->game->FindExtraTag("Bib") != nullptr) {
      Tcl_AppendResult(
          ti, "  <red><run ::Bibliography::ShowRef>Bib</run></red>", nullptr);
    }
  ...
  Tcl_AppendResult(ti, translate(ti, "LastMove", "Last move"), nullptr);
  Tcl_AppendResult(ti, ": <darkblue>", temp, "</darkblue>", nullptr);
  nags = db->game->GetNags();
  if (printNags && *nags != 0 && !hideNextMove) {
    Tcl_AppendResult(ti, "<red>", nullptr);
    for (uint nagCount = 0; nags[nagCount] != 0; nagCount++) {
      ...
      if (nagCount > 0 || (nagstr[0] != '!' && nagstr[0] != '?')) {
        Tcl_AppendResult(ti, " ", nullptr);
      }
      Tcl_AppendResult(ti, nagstr, nullptr);
    }
    Tcl_AppendResult(ti, "</red>", nullptr);
  }
  ...
  Tcl_AppendResult(ti, "   ", translate(ti, "NextMove", "Next"), nullptr);
  Tcl_AppendResult(ti, ": <darkblue>", temp, "</darkblue>", nullptr);
  ...
  if (!hideNextMove && varCount > 0) {
    Tcl_AppendResult(ti, "<br>", translate(ti, "Variations"), ":", nullptr);
    for (uint vnum = 0; vnum < varCount && vnum < 5; vnum++) {
      ...
      std::snprintf(temp, sizeof(temp),
                    ...
                    vnum + 1);
      Tcl_AppendResult(ti, "<green>", temp, "</green>: ", nullptr);
      if (s[0] == 0) {
      ...
      if (*firstNag >= NAG_GoodMove && *firstNag <= NAG_DubiousMove) {
        ...
        Tcl_AppendResult(ti, "<red>", s, "</red>", nullptr);
      }
      Tcl_AppendResult(ti, "</run>", nullptr);
  ...
  if (db->game->GetMoveComment() != nullptr) {
    Tcl_AppendResult(ti, "<br>", translate(ti, "Comment"),
                     " <green><run makeCommentWin>", nullptr);
    ...
    while (lines < commentHeight && *s != 0) {
      ...
      while (lineLen < commentWidth && *s != 0 && *s != '\n') {
        char ch = *s;
        if (ch == '<') {
          Tcl_AppendResult(ti, "<lt>", nullptr);
        } else if (ch == '>') {
          Tcl_AppendResult(ti, "<gt>", nullptr);
        } else {
      ...
      if (*s != 0 && lines < commentHeight) {
        Tcl_AppendResult(ti, "<br>", nullptr);
      }
    }

    Tcl_AppendResult(ti, "</run></green>", nullptr);

    // Add continuation of comment if there is more
    if (*s != 0) {
      Tcl_AppendResult(ti, "<br><green><run makeCommentWin>", nullptr);
  ...
  if (ecoBook) {
    ...
    if (!ecoStr.empty()) {
      ...
      Tcl_AppendResult(ti, "<br>ECO:  <blue><run ::windows::eco::Refresh ",
                       estr, ">", tempDStr.Data(), "</run></blue>", nullptr);
    }
  }
  if (showFEN) {
    char boardStr[200];
    db->game->GetCurrentPos()->PrintFEN(boardStr);
    Tcl_AppendResult(ti, "<br><gray>", boardStr, "</gray>", nullptr);
  }
  return TCL_OK;
}
...
int sc_game_merge(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  if (base == 0)
    return UI_Result(ti, ERROR_FileNotOpen);
  ...
  while (ply < endPly) {
    ...
    if (mergeMove == nullptr) {
      break;
    }
    if (db->game->AddMove(*mergeMove) != OK) {
      break;
    }
    ply++;
  }
...
int sc_game_moves(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  for (uint i = plyCount; i > 0; i--, count++) {
    ...
    if (listFormat) {
      ...
      Tcl_AppendResult(ti, (count == 0 ? "" : " "), move, nullptr);
    }
  }
  delete[] moveStrings;
  return TCL_OK;
...
int sc_game_novelty(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  const char *usage = "Usage: sc_game novelty [-older] base";

  bool olderGamesOnly = false;

  int baseArg = 2;
  if (argc >= baseArg && argv[baseArg][0] == '-' && argv[baseArg][1] == 'o' &&
  ...
  while (g->MoveForward() == OK) {
    ...
    for (uint i = 0, n = base->numGames(); i < n; i++) {
      ...
      if (olderGamesOnly) {
        if (base->getIndexEntry(i)->GetDate() >= currentDate)
          continue;
      }
      if (count++ != 0)
        break;
...
//        active game.
//      -unicode (0|1): use unicocde characters (e.g. U+2654 for king).
//      Default=0.
int sc_game_pgn(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  static const char *options[] = {
      "-column",  "-comments",    "-base",           "-gameNumber",
      "-format",  "-shortHeader", "-indentComments", "-indentVariations",
      "-symbols", "-tags",        "-variations",     "-width",
      "-space",   "-markCodes",   "-unicode",        "-showDiagram",
      nullptr};
  enum {
    OPT_COLUMN,
    OPT_COMMENTS,
  ...
  while (thisArg < argc) {
    ...
    if (index == -1) {
      Tcl_AppendResult(ti, "Invalid option to sc_game pgn: ", argv[thisArg],
                       "; valid options are: ", nullptr);
      for (const char **s = options; *s != nullptr; s++) {
        Tcl_AppendResult(ti, *s, " ", nullptr);
      }
      return TCL_ERROR;
    }
    ...
    if (thisArg + 1 == argc) {
      Tcl_AppendResult(ti, "Invalid option value: sc_game pgn ", options[index],
                       " requires a value.", nullptr);
      return TCL_ERROR;
    }
  ...
  Tcl_AppendResult(ti, pgnBuf.first, nullptr);
  return TCL_OK;
}
...
//    a setup position.
int sc_game_startBoard(ClientData, Tcl_Interp *ti, int argc,
                       const char **argv) {
  if (argc == 2) {
...
int sc_game_tags(ClientData cd, Tcl_Interp *ti, int argc, const char **argv) {
  const char *options[] = {"get", "set", "reload", "share", nullptr};
...
int sc_game_tags_get(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  static const char *options[] = {
      ...
      "Round",      "White",      "Black", "Result", "WhiteElo", "BlackElo",
      "WhiteRType", "BlackRType", "ECO",   "EDate",  "EYear",    "EMonth",
      "EDay",       "Extra",      nullptr};
  enum {
    T_Event,
    T_Site,
  ...
  case T_ECO: {
    ...
    Tcl_AppendResult(ti, ecoStr, nullptr);
    break;
  }
...
static uint strGetRatingType(const char *name) {
  uint i = 0;
  while (ratingTypeNames[i] != nullptr) {
...
int sc_game_tags_set(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  while (arg + 1 < argc) {
    ...
    case T_EXTRA: {
      ...
      Tcl_Obj **objv;
      // Usage :: sc_game tags set -extra [ list "Annotator \"boob [sc_pos
      // moveNumber]\"" ]
...
int sc_game_tags_share(ClientData, Tcl_Interp *ti, int argc,
                       const char **argv) {
  const char *usage =
...
//////////////////////////////////////////////////////////////////////
///  INFO functions
static bool date_ValidString(const char *str) {
...
int sc_info(ClientData cd, Tcl_Interp *ti, int argc, const char **argv) {
  static const char *options[] = {
      "clipbase", "decimal",   "priority", "html",     "limit", "ratings",
      "suffix",   "validDate", "version",  "language", nullptr};
  enum {
    INFO_CLIPBASE,
    INFO_DECIMAL,
    INFO_PRIORITY,
  ...
  case INFO_LANGUAGE:
    ...
    if (strcmp(argv[2], "it") == 0) {
      language = 4;
    }
    if (strcmp(argv[2], "ne") == 0) {
      language = 5;
    }
    if (strcmp(argv[2], "cz") == 0) {
      language = 6;
    }
    ...
    if (strcmp(argv[2], "sw") == 0) {
      language = 9;
    }
    if (strcmp(argv[2], "ca") == 0) {
      language = 10;
    }
    if (strcmp(argv[2], "fi") == 0) {
      language = 11;
    }
...
int sc_info_limit(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  static const char *options[] = {"elo", "year", "bases", "epd", nullptr};
...
static int sc_epd_open(Tcl_Interp* ti, int argc, const char** argv, bool create) {
	...
	if (err != OK) {
		delete pb;
		epdBooks[freeID] = nullptr;
		Tcl_AppendResult(ti, "Unable to ",
		                 create ? "create" : "open",
		                 " EPD file: ", filename, nullptr);
		return TCL_ERROR;
	}
...
int sc_epd(ClientData cd, Tcl_Interp* ti, int argc, const char** argv) {
	...
	if (epdID < 0 || epdID >= MAX_EPD || epdBooks[epdID] == nullptr) {
		Tcl_AppendResult(ti, "Error: sc_epd ", options[index],
		                 ": invalid EPD ID number", nullptr);
		return TCL_ERROR;
	}
	...
	case EPD_GET: {
		...
		if (epdBooks[epdID]->find(db->game->GetCurrentPos(), &text)) {
			Tcl_AppendResult(ti, text, nullptr);
		}
		break;
	}
	...
	case EPD_NAME:
		Tcl_AppendResult(ti, epdBooks[epdID]->getFileName(), nullptr);
		break;
...
int sc_pos(ClientData cd, Tcl_Interp *ti, int argc, const char **argv) {
  static const char *options[] = {
      ...
      "attacks",   "getPrevComment", "coordToSAN", "isMate", nullptr};
  enum {
    POS_ADDNAG,
    POS_ANALYZE,
    POS_BESTSQ,
  ...
  case POS_FEN:
    db->game->GetCurrentPos()->PrintFEN(boardStr);
    Tcl_AppendResult(ti, boardStr, nullptr);
    break;
    ...
    for (colorT c = WHITE; c <= BLACK; c++) {
      for (uint i = 0; i < pos.GetCount(c); i++) {
        ...
        if (att) {
          ...
          if (att > 1)
            Tcl_AppendElement(ti, "green");
          else if (att > 0)
            Tcl_AppendElement(ti, "yellow");
          else
            Tcl_AppendElement(ti, "red");
        }
      }
    }
  } break;
...
int sc_pos_analyze(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  static const char *options[] = {"-time",        "-hashkb",  "-pawnkb",
                                  "-post",        "-pruning", "-mindepth",
                                  "-searchdepth", nullptr};
  enum {
    OPT_TIME,
    OPT_HASH,
  ...
  while (arg + 1 < argc) {
    ...
    switch (index) {
    case OPT_TIME:
      searchTime = strGetUnsigned(value);
      break;
    case OPT_HASH:
      hashTableKB = strGetUnsigned(value);
      break;
    case OPT_PAWN:
      pawnTableKB = strGetUnsigned(value);
      break;
    case OPT_POST:
      postMode = strGetBoolean(value);
      break;
    case OPT_PRUNING:
      pruning = strGetBoolean(value);
      break;
    case OPT_MINDEPTH:
      mindepth = strGetUnsigned(value);
      break;
    case OPT_SEARCHDEPTH:
      searchdepth = strGetUnsigned(value);
      break;
    default:
    ...
    if (ecoBook != nullptr) {
      for (uint i = 0; i < mlist.Size(); i++) {
        ...
        if (eco >= bestEco) {
          ...
          bestEco = eco;
          std::rotate(mlist.begin(), mlist.begin() + i, mlist.begin() + i + 1);
        }
      }
    }
...
int sc_pos_isAt(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  static const char *options[] = {"start", "end", "vstart", "vend", nullptr};
  enum { OPT_START, OPT_END, OPT_VSTART, OPT_VEND };
  int index = -1;
  if (argc == 3) {
    index = strUniqueMatch(argv[2], options);
  }
...
int sc_pos_isLegal(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  auto pos = db->game->GetCurrentPos();
  bool legal =
      pos->IsLegalMove(sq1, sq2, EMPTY) || pos->IsLegalMove(sq2, sq1, EMPTY) ||
      pos->IsLegalMove(sq1, sq2, QUEEN) || pos->IsLegalMove(sq2, sq1, QUEEN);
...
int sc_pos_setComment(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  if (str[0] == 0 || (isspace((char)str[0]) && str[1] == 0)) {
    ...
    if (oldComment != nullptr) {
      db->game->SetMoveComment(nullptr);
      db->gameAltered = true;
    }
  } else {
    // Only set the comment if it has actually changed:
    if (oldComment == nullptr || !strEqual(str, oldComment)) {
...
int sc_name_correct(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  if (!NameBase::IsValidNameType(nt)) {
    return errorResult(ti, "Usage: sc_name correct p|e|s|r <corrections>");
  }
...
int sc_name_edit(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  const char *options[] = {"player", "event", "site",  "round",
                           "rating", "date",  "edate", nullptr};
  enum {
    OPT_PLAYER,
    OPT_EVENT,
    OPT_SITE,
...
UI_res_t sc_name_retrievename(UI_handle_t ti, const SpellChecker &sp, int argc,
                              const char **argv) {
...
int sc_name_info(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  static char *lastPlayerName = nullptr;
  ...
  if (db->getNameBase()->FindExactName(NAME_PLAYER, playerName, &id) != OK) {
    Tcl_AppendResult(ti, "The name \"", playerName,
                     "\" does not exist in this database.", nullptr);
    return TCL_OK;
  }

  // Try to find opponent in this database:
  idNumberT opponentId = 0;
  const char *opponent = nullptr;
  ...
  if (opponent != nullptr) {
    if (db->getNameBase()->FindExactName(NAME_PLAYER, opponent, &opponentId) !=
        OK) {
      opponent = nullptr;
    }
  }
  ...
  for (uint i = 0, n = db->numGames(); i < n; i++) {
    ...
    if (whiteId == id) {
      ...
      if (opponent != nullptr && blackId == opponentId) {
      ...
      if (opponent != nullptr && whiteId == opponentId) {
  ...
  Tcl_AppendResult(ti, startBold, playerName, endBold, newline, nullptr);
  ...
  if (spChecker != nullptr) {
    ...
    if (pInfo) {
      Tcl_AppendResult(ti, "  ", pInfo->GetComment(), newline, nullptr);
    }
  }
  sprintf(temp, "  %s%u%s %s (%s: %u)",
          htextOutput
              ...
              : "",
          totalcount[STATS_ALL], htextOutput ? "</run></red>" : "",
          (totalcount[STATS_ALL] == 1 ? translate(ti, "game")
                                      : translate(ti, "games")),
          translate(ti, "Filter"), totalcount[STATS_FILTER]);
  Tcl_AppendResult(ti, temp, nullptr);
  ...
  Tcl_AppendResult(ti, newline, nullptr);
      ...
      for (size_t i = 0, n = bio.size(); i < n; i++) {
        if (i == 0) {
          Tcl_AppendResult(ti, newline, startHeading,
                           translate(ti, "Biography"), ":", endHeading, newline,
                           nullptr);
        }
        Tcl_AppendResult(ti, "  ", bio[i], newline, nullptr);
      }
    }
  }
  ...
  Tcl_AppendResult(ti, newline, startHeading, temp, ":", endHeading, newline,
                   nullptr);
  ...
  sprintf(
      temp, fmt, htextOutput ? "<tt>" : "", wbtWidth, translate(ti, "Black:"),
      percent / 100, decimalPointChar, percent % 100,
      htextOutput ? "<red><run sc_name info -fW {}; ::windows::stats::Refresh>"
                  : "",
      blackscore[STATS_ALL][RESULT_White], htextOutput ? "</run></red>" : "",
      htextOutput ? "<red><run sc_name info -fD {}; ::windows::stats::Refresh>"
                  : "",
      blackscore[STATS_ALL][RESULT_Draw], htextOutput ? "</run></red>" : "",
      htextOutput ? "<red><run sc_name info -fL {}; ::windows::stats::Refresh>"
                  : "",
      blackscore[STATS_ALL][RESULT_Black], htextOutput ? "</run></red>" : "",
      score / 2, decimalPointChar, score % 2 ? '5' : '0',
      htextOutput ? "<red><run sc_name info -fA {}; ::windows::stats::Refresh>"
                  : "",
      blackcount[STATS_ALL], htextOutput ? "</run></red></tt>" : "");
  Tcl_AppendResult(ti, temp, newline, nullptr);
  ...
  sprintf(
      temp, fmt, htextOutput ? "<tt>" : "", wbtWidth, translate(ti, "Total:"),
      percent / 100, decimalPointChar, percent % 100,
      htextOutput ? "<red><run sc_name info -fwW {}; ::windows::stats::Refresh>"
                  : "",
      bothscore[STATS_ALL][RESULT_White], htextOutput ? "</run></red>" : "",
      htextOutput ? "<red><run sc_name info -fdD {}; ::windows::stats::Refresh>"
                  : "",
      bothscore[STATS_ALL][RESULT_Draw], htextOutput ? "</run></red>" : "",
      htextOutput ? "<red><run sc_name info -flL {}; ::windows::stats::Refresh>"
                  : "",
      bothscore[STATS_ALL][RESULT_Black], htextOutput ? "</run></red>" : "",
      score / 2, decimalPointChar, score % 2 ? '5' : '0',
      htextOutput ? "<red><run sc_name info -faA {}; ::windows::stats::Refresh>"
                  : "",
      totalcount[STATS_ALL], htextOutput ? "</run></red></tt>" : "");
  Tcl_AppendResult(ti, temp, newline, nullptr);
  ...
  sprintf(temp, fmt, htextOutput ? "<tt>" : "", wbtWidth,
          translate(ti, "White:"), percent / 100, decimalPointChar,
          percent % 100, "", whitescore[STATS_FILTER][RESULT_White], "", "",
          whitescore[STATS_FILTER][RESULT_Draw], "", "",
          whitescore[STATS_FILTER][RESULT_Black], "", score / 2,
          decimalPointChar, score % 2 ? '5' : '0', "", whitecount[STATS_FILTER],
          htextOutput ? "</tt>" : "");
  Tcl_AppendResult(ti, temp, newline, nullptr);
  ...
  sprintf(temp, fmt, htextOutput ? "<tt>" : "", wbtWidth,
          translate(ti, "Black:"), percent / 100, decimalPointChar,
          percent % 100, "", blackscore[STATS_FILTER][RESULT_White], "", "",
          blackscore[STATS_FILTER][RESULT_Draw], "", "",
          blackscore[STATS_FILTER][RESULT_Black], "", score / 2,
          decimalPointChar, score % 2 ? '5' : '0', "", blackcount[STATS_FILTER],
          htextOutput ? "</tt>" : "");
  Tcl_AppendResult(ti, temp, newline, nullptr);
  ...
  sprintf(temp, fmt, htextOutput ? "<tt>" : "", wbtWidth,
          translate(ti, "Total:"), percent / 100, decimalPointChar,
          percent % 100, "", bothscore[STATS_FILTER][RESULT_White], "", "",
          bothscore[STATS_FILTER][RESULT_Draw], "", "",
          bothscore[STATS_FILTER][RESULT_Black], "", score / 2,
          decimalPointChar, score % 2 ? '5' : '0', "", totalcount[STATS_FILTER],
          htextOutput ? "</tt>" : "");
  Tcl_AppendResult(ti, temp, newline, nullptr);
  ...
  if (opponent != nullptr) {
    Tcl_AppendResult(ti, newline, startHeading, translate(ti, "PInfoAgainst"),
                     " ", startBold, opponent, endBold, ":", endHeading,
                     newline, nullptr);
    ...
    sprintf(
        ...
        htextOutput
            ...
            : "",
        whitescore[STATS_OPP][RESULT_White], htextOutput ? "</run></red>" : "",
        htextOutput
            ? "<red><run sc_name info -od {}; ::windows::stats::Refresh>"
            : "",
        whitescore[STATS_OPP][RESULT_Draw], htextOutput ? "</run></red>" : "",
        htextOutput
            ? "<red><run sc_name info -ol {}; ::windows::stats::Refresh>"
            : "",
        whitescore[STATS_OPP][RESULT_Black], htextOutput ? "</run></red>" : "",
        score / 2, decimalPointChar, score % 2 ? '5' : '0',
        htextOutput
            ? "<red><run sc_name info -oa {}; ::windows::stats::Refresh>"
            : "",
        whitecount[STATS_OPP], htextOutput ? "</run></red></tt>" : "");
    Tcl_AppendResult(ti, temp, newline, nullptr);
    ...
    sprintf(
        ...
        htextOutput
            ? "<red><run sc_name info -oW {}; ::windows::stats::Refresh>"
            : "",
        blackscore[STATS_OPP][RESULT_White], htextOutput ? "</run></red>" : "",
        htextOutput
            ? "<red><run sc_name info -oD {}; ::windows::stats::Refresh>"
            : "",
        blackscore[STATS_OPP][RESULT_Draw], htextOutput ? "</run></red>" : "",
        htextOutput
            ? "<red><run sc_name info -oL {}; ::windows::stats::Refresh>"
            : "",
        blackscore[STATS_OPP][RESULT_Black], htextOutput ? "</run></red>" : "",
        score / 2, decimalPointChar, score % 2 ? '5' : '0',
        htextOutput
            ? "<red><run sc_name info -oA {}; ::windows::stats::Refresh>"
            : "",
        blackcount[STATS_OPP], htextOutput ? "</run></red></tt>" : "");
    Tcl_AppendResult(ti, temp, newline, nullptr);
    ...
    sprintf(
        ...
        htextOutput
            ...
            : "",
        bothscore[STATS_OPP][RESULT_White], htextOutput ? "</run></red>" : "",
        htextOutput
            ? "<red><run sc_name info -odD {}; ::windows::stats::Refresh>"
            : "",
        bothscore[STATS_OPP][RESULT_Draw], htextOutput ? "</run></red>" : "",
        htextOutput
            ? "<red><run sc_name info -olL {}; ::windows::stats::Refresh>"
            : "",
        bothscore[STATS_OPP][RESULT_Black], htextOutput ? "</run></red>" : "",
        score / 2, decimalPointChar, score % 2 ? '5' : '0',
        htextOutput
            ? "<red><run sc_name info -oaA {}; ::windows::stats::Refresh>"
            : "",
        totalcount[STATS_OPP], htextOutput ? "</run></red></tt>" : "");
    Tcl_AppendResult(ti, temp, newline, nullptr);
  }
  ...
  for (color = WHITE; color <= BLACK; color++) {
    for (uint count = 0; count < 6; count++) {
      ...
      if (mostPlayed > 0) {
        if (count == 0) {
          const char *s =
              (color == WHITE ? "PInfoMostWhite" : "PInfoMostBlack");
          Tcl_AppendResult(ti, newline, startHeading, translate(ti, s), ":",
                           endHeading, newline, nullptr);
        } else if (count == 3) {
          Tcl_AppendResult(ti, newline, nullptr);
        }
        Tcl_AppendResult(ti, "   ", nullptr);
        ...
        temp[2] = 0;
        if (htextOutput) {
          Tcl_AppendResult(ti, "<blue><run ::windows::eco::Refresh ", temp, ">",
                           nullptr);
        }
        Tcl_AppendResult(ti, temp, nullptr);
        if (htextOutput) {
          Tcl_AppendResult(ti, "</run></blue>", nullptr);
        }
...
struct PlayerActivity {
  ...
  void addDate(dateT date) {
    if (firstDate == ZERO_DATE || date < firstDate)
      firstDate = date;
    if (date > lastDate)
      lastDate = date;
  }
...
public:
  PListSort(scidBaseT *dbase, const std::vector<PlayerActivity> &activity,
            int sortOrder)
  ...
  bool operator()(idNumberT p1, idNumberT p2) {
    ...
    case SORT_ELO:
      ...
      break;
    case SORT_GAMES:
      compare = dbase_->getNameFreq(NAME_PLAYER, p2) -
                dbase_->getNameFreq(NAME_PLAYER, p1);
      break;
    case SORT_OLDEST:
      // Sort by oldest game year in ascending order:
      compare = date_GetYear(activity_[p1].firstDate) -
                date_GetYear(activity_[p2].firstDate);
      break;
    case SORT_NEWEST:
      // Sort by newest game date in descending order:
      compare = date_GetYear(activity_[p2].lastDate) -
                date_GetYear(activity_[p1].lastDate);
      break;
    }
...
int sc_name_plist(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  static const char *options[] = {"-name",     "-minElo",   "-maxElo",
                                  "-minGames", "-maxGames", "-size",
                                  "-sort",     nullptr};
  enum {
    OPT_NAME,
    OPT_MINELO,
  ...
  // Valid sort types:
  static const char *sortModes[] = {"elo",    "games", "oldest",
                                    "newest", "name",  nullptr};
  enum { SORT_ELO, SORT_GAMES, SORT_OLDEST, SORT_NEWEST, SORT_NAME };
  ...
  std::partial_sort(plist.begin(), plist.begin() + count, plist.end(),
...
UI_res_t sc_name_ratings(UI_handle_t ti, scidBaseT &dbase,
                         const SpellChecker &sp, int argc, const char **argv) {
  const char *options[] = {"-update", "-test", "-change", "-filter"};
  enum { OPT_UPDATE, OPT_TEST, OPT_CHANGE, OPT_FILTER };
  ...
  if (!sp.hasEloData()) {
    return UI_Result(ti, ERROR,
                     "The current spellcheck file does not have "
  ...
  std::vector<const PlayerElo *> vElo(nb->namebase_size(NAME_PLAYER), nullptr);
  ...
  auto entry_op = [&](IndexEntry &ie) {
    dateT date = ie.GetDate();
    eloT eloWhite =
        (!overwrite && ie.GetWhiteElo() != 0) ? 0 : getElo(ie.GetWhite(), date);
    eloT eloBlack =
        (!overwrite && ie.GetBlackElo() != 0) ? 0 : getElo(ie.GetBlack(), date);
...
static void checkSpellChkReady(bool block) {
  ...
  spellChkLoading.store(false);
  if (newChk == nullptr) return;

  if (spellChk != nullptr) delete spellChk;
  spellChk = newChk;
}
...
int sc_name_read(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  for (nameT i = 0; i < NUM_NAME_TYPES; i++) {
    size_t n = (spellChk == nullptr) ? 0 : spellChk->numCorrectNames(i);
    res.push_back(n);
  }
  return UI_Result(ti, OK, res);
}
...
UI_res_t sc_name_spellcheck(UI_handle_t ti, scidBaseT &dbase,
                            const SpellChecker &sp, int argc,
                            const char **argv) {
  nameT nt = NAME_INVALID;
  ...
  bool ambiguous = true;
  const char *usage =
      "Usage: sc_name spellcheck [-max <integer>] [-surnames <boolean>] "
      "[-ambiguous <boolean>] players|events|sites|rounds";

  const char *options[] = {"-max", "-surnames", "-ambiguous", nullptr};
  enum { OPT_MAX, OPT_SURNAMES, OPT_AMBIGUOUS };
  ...
  for (idNumberT id = 0, n = nb->GetNumNames(nt); id < n; id++) {
    ...
    if (nt == NAME_PLAYER && !doSurnames && strIsSurnameOnly(origName))
      continue;
    ...
    if (!ambiguous && corrections.size() > 1)
      continue;
  ...
  if (nt == NAME_PLAYER && !doSurnames) {
    res += "Note: player names with a surname only, such as \"Kramnik\",\n"
...
UI_res_t sc_name(UI_extra_t cd, UI_handle_t ti, int argc, const char **argv) {
  static const char *options[] = {
      "correct", "edit",       "info",         "match", "plist", "ratings",
      "read",    "spellcheck", "retrievename", "elo",   nullptr};
  enum {
    OPT_CORRECT,
    OPT_EDIT,
    OPT_INFO,
...
static uint avgGameLength(resultT result) {
  uint sum = 0;
  uint count = 0;
  for (gamenumT i = 0, n = db->numGames(); i < n; i++) {
    const IndexEntry *ie = db->getIndexEntry(i);
...
int sc_report(ClientData cd, Tcl_Interp *ti, int argc, const char **argv) {
  static const char *options[] = {
      "avgLength", "best",        "counts", "create",    "eco",
      "elo",       "endmaterial", "format", "frequency", "line",
      "max",       "moveOrders",  "notes",  "players",   "print",
      "score",     "select",      "themes", nullptr};
  enum {
    OPT_AVGLENGTH,
    OPT_BEST,
    OPT_COUNTS,
  ...
  DString *dstr = nullptr;
  int index = strUniqueMatch(argv[2], options);

  if (!db->inUse) {
    return errorResult(ti, errMsgNotOpen(ti));
  }
  if (index >= 0 && index != OPT_CREATE && report == nullptr) {
  ...
  case OPT_BEST:
    if (argc != 5) {
      return errorResult(
          ti, "Usage: sc_report opening|player best w|b|a|o|n <count>");
    }
    dstr = new DString;
  ...
  case OPT_ECO:
    if (argc > 3) {
      ...
      Tcl_AppendResult(ti, report->GetEco(), nullptr);
    }
    break;
  ...
  case OPT_FORMAT:
    if (argc != 4) {
      return errorResult(
          ti, "Usage: sc_report opening|player format latex|html|text|ctext");
    }
  ...
  case OPT_NOTES:
    if (argc < 4 || argc > 5) {
      return errorResult(
          ti, "Usage: sc_report opening|player notes <0|1> [numrows]");
    }
    report->ClearNotes();
  ...
  case OPT_PRINT:
    ...
    dstr = new DString;
    report->PrintTable(dstr, argc > 4 ? argv[4] : "", argc > 5 ? argv[5] : "");
    Tcl_AppendResult(ti, dstr->Data(), nullptr);
    break;
  ...
  case OPT_SCORE:
    appendUintElement(ti, report->PercentScore());
    {
      ...
      percent = percent * 500;
      uint sum = (stats.nResults[RESULT_White] + stats.nResults[RESULT_Draw] +
...
int sc_report_create(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  static const char *usage = "Usage: sc_report opening|player create "
                             "[maxExtraMoves] [maxLines] [excludeMove]";
  ...
  if (reports[reportType] != nullptr) {
    delete reports[reportType];
  }
  OpTable *report =
      new OpTable(reportTypeName[reportType], db->game, ecoBook.get());
  ...
  for (uint gnum = 0, n = db->numGames(); gnum < n; gnum++) {
    ...
    if (ply != 0) {
      ...
      if (ply != 0) {
        ...
        OpLine *line = new OpLine(scratchGame, ie, gnum + 1, maxExtraMoves,
                                  maxThemeMoveNumber);
        if (report->Add(line)) {
...
int sc_tree_stats(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  auto calc_san = [&](auto const &moves) {
    ...
    std::string result;
    static const size_t MAX_RESULT = 1024;
...
int sc_tree_cachesize(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  if (base)
    base->treeCache.CacheResize(strGetUnsigned(argv[3]));
  return TCL_OK;
}
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// sc_tree_cacheinfo:
...
int sc_search(ClientData cd, Tcl_Interp *ti, int argc, const char **argv) {
  static const char *options[] = {"board", "header", "material", nullptr};
...
int sc_search_board(Tcl_Interp *ti, const scidBaseT *dbase, HFilter filter,
  ...
  auto search_chunk = [&](size_t begin, size_t end) {
    ...
    for (size_t ci = begin; ci < end; ++ci) {
      ...
      if (useVars) {
        ...
        if (ply == 0 && possibleMatch && g.ExactMatch(pos, nullptr, searchType))
          ply = g.GetCurrentPly() + 1;
        if (ply == 0 && possibleFlippedMatch &&
            g.ExactMatch(posFlip, nullptr, searchType))
          ply = g.GetCurrentPly() + 1;
  ...
  sprintf(temp, "%lu / %lu  (%d%c%02d s)",
          ...
          decimalPointChar, centisecs % 100);
  Tcl_AppendResult(ti, temp, nullptr);
  return TCL_OK;
}
...
errorT parsePattern(const char *str, patternT *patt) {
  ASSERT(str != nullptr && patt != nullptr);
...
int sc_search_material(ClientData, Tcl_Interp *ti, int argc,
                       const char **argv) {
  if (!db->inUse) {
  ...
  const char *options[] = {"wq",     "bq",      "wr",   "br",      "wb",
                           "bb",     "wn",      "bn",   "wp",      "bp",
                           "wm",     "bm",      "flip", "filter",  "range",
                           "length", "bishops", "diff", "pattern", nullptr};
  enum {
    OPT_WQ,
    OPT_BQ,
    OPT_WR,
  ...
  while (arg + 1 < argc) {
    ...
    case OPT_PATTERN:
      if (parsePattern(value, &tempPatt) != OK) {
        Tcl_AppendResult(ti, "Invalid pattern: ", value, nullptr);
        return TCL_ERROR;
      }
      ...
      if (!tempPatt.flag && piece_Type(tempPatt.pieceMatch) == PAWN &&
          tempPatt.rankMatch == NO_RANK && tempPatt.fyleMatch != NO_FYLE) {
  ...
  int checkMsig = 1;
  if (max[WQ] > 3 || max[BQ] > 3 || max[WR] > 3 || max[BR] > 3 || max[WB] > 3 ||
      max[BB] > 3 || max[WN] > 3 || max[BN] > 3) {
  ...
  for (; gameNum < n; gameNum++) {
    ...
    // material signature:
    if (checkMsig &&
        !matsig_isReachable(msig, ie->GetFinalMatSig(), ie->GetPromotionsFlag(),
                            ie->GetUnderPromoFlag())) {
      possibleMatch = false;
    }
    if (flip && checkMsig &&
    ...
    if (possibleMatch) {
      ...
      result = g->MaterialMatch(
          hasPromo, bbuf_clone, min, max, patt.data(), patt.size(), minPly,
          maxPly, matchLength, oppBishops, sameBishops, matDiff[0], matDiff[1]);
    }
    if (result == 0 && possibleFlippedMatch) {
      bool hasPromo = ie->GetPromotionsFlag() || ie->GetUnderPromoFlag();
      result = g->MaterialMatch(hasPromo, bbuf, minFlipped, maxFlipped,
                                flippedPatt.data(), flippedPatt.size(), minPly,
                                maxPly, matchLength, oppBishops, sameBishops,
                                matDiff[0], matDiff[1]);
    }
  ...
  if (gameNum != n) {
    Tcl_AppendResult(ti, errMsgSearchInterrupted(ti), "  ", nullptr);
  }
  sprintf(temp, "%lu / %lu  (%d%c%02d s)",
          static_cast<unsigned long>(filter->size()),
          static_cast<unsigned long>(startFilterCount), centisecs / 100,
...
const char *titleStr[NUM_TITLES] = {"gm",  "im",  "fm", "wgm",
                                    "wim", "wfm", "w",  "none"};
...
int sc_search_header(ClientData, Tcl_Interp *ti, scidBaseT *base,
  ...
  bool *wTitles = nullptr;
  bool *bTitles = nullptr;

  bool wToMove = true;
  bool bToMove = true;

  Tcl_Size pgnTextCount = 0;
  const char **sPgnText = nullptr;

  const char *options[] = {"wtitles", "btitles", "toMove", "pgn", nullptr};
  enum { OPT_WTITLES, OPT_BTITLES, OPT_TOMOVE, OPT_PGN };
  ...
  while (arg + 1 < argc) {
    ...
    case OPT_PGN:
      if (Tcl_SplitList(ti, (char *)value, &pgnTextCount, &sPgnText) !=
          TCL_OK) {
        delete[] wTitles;
        delete[] bTitles;
        return TCL_ERROR;
  ...
  checkSpellChkReady(false);
  if (wTitles != nullptr && spellChk != nullptr) {
  ...
  if (bTitles != nullptr && spellChk != nullptr) {
    bool allTitlesOn = true;
    for (uint t = 0; t < NUM_TITLES; t++) {
      if (!bTitles[t]) {
        allTitlesOn = false;
  ...
  if (mWhite.empty() && mBlack.empty() && wToMove == true && bToMove == true &&
      pgnTextCount == 0) {
    skipSearch = true;
  }
...
int sc_var(ClientData cd, Tcl_Interp *ti, int argc, const char **argv) {
  static const char *options[] = {"count", "number",   "create",  "delete",
                                  "enter", "exit",     "first",   "level",
                                  "list",  "moveInto", "promote", nullptr};
  enum {
    VAR_COUNT,
    VAR_NUMBER,
    VAR_CREATE,
...
int sc_book(ClientData cd, Tcl_Interp *ti, int argc, const char **argv) {
  static const char *options[] = {"load",        "close",  "moves", "positions",
                                  "movesupdate", "update", nullptr};
  enum {
    BOOK_LOAD,
    BOOK_CLOSE,
    BOOK_MOVE,
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// sc_book_moves:
//    returns a list of all moves contained in opened book and their probability
//    in a TCL list
...
int sc_book_positions(ClientData, Tcl_Interp *ti, int argc, const char **argv) {
  ...
  Tcl_AppendResult(ti, moves, nullptr);
  return TCL_OK;
}
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// sc_book_update:
...
int sc_book_movesupdate(ClientData, Tcl_Interp *ti, int argc,
                        const char **argv) {
  if (argc != 6) {
    return errorResult(
        ti, "Usage: sc_book movesupdate <moves> <probs> slot tempfile");
  }
  uint slot = strGetUnsigned(argv[4]);
  scid_book_movesupdate((char *)argv[2], (char *)argv[3], slot,
                        (char *)argv[5]);
  return TCL_OK;
}
//////////////////////////////////////////////////////////////////////
